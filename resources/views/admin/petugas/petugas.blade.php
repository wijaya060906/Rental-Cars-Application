    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
          <h4 class="card-title">Data User (Petugas)</h4>
          <a href="{{ route('admin.petugas.create') }}" class="btn btn-success">Create</a>
        </div>
        <div class="card-body">
            ...
            <table id="basic-datatables" class="display table table-striped table-hover">
              ...
              <tbody>
                @foreach($users as $user)
                  @if($user->lvl === 'petugas') <!-- Only display users with 'petugas' role -->
                    <tr>
                      <td>{{ $user->name }}</td>
                      <td>{{ $user->email }}</td>
                      <td>{{ $user->lvl }}</td>
